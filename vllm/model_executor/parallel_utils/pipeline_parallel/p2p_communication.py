...
def _communicate_shapes(tensor_send_next, tensor_send_prev, recv_prev, recv_next, config):
    ...
    if recv_prev:
        recv_prev_shape_tensor = torch.empty(
            (2), device=torch.cuda.current_device(), dtype=torch.int64
        )
    if recv_next:
...
def _communicate(
    *,
    ...
    Arguments:
        ...
        tensor_shape (List[int] or torch.Size, optional):
    ...
    NOTE:
        This function only support simple send recv now for simplicity in vllm-batching.
...
def recv_forward(tensor_shape: Shape, config: ParallelConfig) -> torch.Tensor:
...
def send_forward(output_tensor: torch.Tensor, config: ParallelConfig) -> None:
