...
def sample_requests(
    ...
    output_lens = [t[2] for t in sampled_requests]
    max_output_len = max(output_lens)
    print(sum(prompt_lens) / len(prompt_lens), sum(output_lens) / len(output_lens), max_output_len)
...
def run_vllm(
    ...
    if exec_type == 1:
        ...
        num_model_per_group = num_models
    llm = LLM(
        model=model,
    ...
    llm._run_engines(use_tqdm=True)
    end = time.time()
    return end - start
